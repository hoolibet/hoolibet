...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total hoolibet balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in hoolibet/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in hoolibet/kb\n"
    ...
    /* hbet UniValue zhbetObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zhbetObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zhbetObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zHBETsupply", zhbetObj)); */
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"hoolibetaddress\"\n"
            "\nReturn information about the given hoolibet address.\n"

            "\nArguments:\n"
            "1. \"hoolibetaddress\"     (string, required) The hoolibet address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"hoolibetaddress\", (string) The hoolibet address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: HooliBet address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are hoolibet addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) hoolibet address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"hoolibetaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"hoolibetaddress\"  (string, required) The hoolibet address to use for the signature.\n"
