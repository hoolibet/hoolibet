...
CzHBETWallet::CzHBETWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhbet seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHBETSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHBETSeed_deprecated()) {
                    LogPrintf("%s: Updated zHBET seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhbet seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhbet wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHBETWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHBETCount(nCountLastUsed);
    else if (!walletdb.ReadZHBETCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHBETWallet::Lock()
{
    seedMaster = 0;
}

void CzHBETWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHBETWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHBET(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHBETWallet::LoadMintPoolFromDB()
{
...
void CzHBETWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHBETWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHBETWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhbetTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHBETWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHBET(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhbetTracker which also adds to database
    pwalletMain->zhbetTracker->Add(dMint, true);
...
void CzHBETWallet::SeedToZHBET(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHBETWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHBETWallet::GenerateDeterministicZHBET(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHBETWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHBETWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
