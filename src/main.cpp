...
#if defined(NDEBUG)
#error "HooliBet cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uhbet) are considered zero fee (for relaying and mining)
...
{
    //Check to see if the zHBET is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zHBET spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zHBET without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zHBET spend with serial %s is already in block %d\n", __func__,
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zHBET spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zHBET spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zhbet");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zHBET mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        ...
        nSubsidy = 0.01 * COIN;
    } else if (nHeight < 22500 && nHeight >= 21000) {
        nSubsidy = 2 * COIN;
	} else if (nHeight < 25000 && nHeight >= 22500) {
        nSubsidy = 1.9 * COIN;
	} else if (nHeight < 27500 && nHeight >= 25000) {
        nSubsidy = 1.7 * COIN;
	} else if (nHeight < 28500 && nHeight >= 27500) {
        nSubsidy = 1.6 * COIN;
	} else if (nHeight < 32500 && nHeight >= 28500) {
        nSubsidy = 1 * COIN;
	} else if (nHeight < 35000 && nHeight >= 32500) {
        nSubsidy = 1.2 * COIN;
	} else if (nHeight < 37500 && nHeight >= 35000) {
        nSubsidy = 2 * COIN;
	} else if (nHeight < 40000 && nHeight >= 37500) {
        nSubsidy = 2.5 * COIN;
	} else if (nHeight < 43000 && nHeight >= 40000) {
        nSubsidy = 2 * COIN;
	} else if (nHeight < 45000 && nHeight >= 43000) {
        nSubsidy = 2.1 * COIN;
	} else if (nHeight < 48000 && nHeight >= 45000) {
        nSubsidy = 2.3 * COIN;
	} else if (nHeight < 53000 && nHeight >= 48000) {
        nSubsidy = 2.4 * COIN;
	} else if (nHeight < 66000 && nHeight >= 53000) {
        nSubsidy = 2.5 * COIN;
	} else if (nHeight < 69000 && nHeight >= 66000) {
        nSubsidy = 3.6 * COIN;
	} else if (nHeight < 300000 && nHeight >= 69000) {
        nSubsidy = 4 * COIN;
	} else {
        nSubsidy = 1 * COIN;
    }
...
int64_t StakeInputMinimal(int nHeight)
{
    int64_t nStakeInputMin = 0;
    ...
    if (nHeight <= Params().LAST_POW_BLOCK()) {
	    ...
	    nStakeInputMin = 3 * COIN;
	} else {
        nStakeInputMin = 10 * COIN;
    }
    
	return nStakeInputMin;
}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK() || blockValue == 0) {
	    ...
	    ret = blockValue * 0.95; // 90% of block reward
	}
	return ret;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from HooliBet
...
{
    RenameThread("hoolibet-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZHBETMinted()
{
...
void RecalculateZHBETSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zHBET supply
...
bool RecalculateHBETSupply(int nHeightStart)
{
...
{
    // HooliBet: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZHBETSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZHBETMinted();
        RecalculateZHBETSpent();
        RecalculateHBETSupply(Params().Zerocoin_StartHeight());
    }

    //Track zHBET money supply in the block index
    if (!UpdateZHBETSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zHBET supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zHbetSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert HBET to zHBET
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.IsProofOfStake()) {
		...
		if (txPrev.vout[block.vtx[1].vin[0].prevout.n].nValue < StakeInputMinimal(chainActive.Height())) {
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zHBET spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zHBET serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zHBET stake block is less than allowed start height", __func__);

    if (CZHbetStake* zHBET = dynamic_cast<CZHbetStake*>(stake)) {
        CBlockIndex* pindexFrom = zHBET->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zHBET stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zHBET stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zHBET->GetValue()));
        if (nChecksum200 != zHBET->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zHBET->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZHBET() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zHBET fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zHBET mints and %d zHBET spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // HooliBet: We use certain sporks during IBD, so check to see if they are
