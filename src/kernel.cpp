...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZHbetStake(spend));
    } else {
        ...
        CHbetStake* hbetInput = new CHbetStake();
        hbetInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(hbetInput);
    }
