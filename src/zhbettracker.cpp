...
CzHBETTracker::CzHBETTracker(std::string strWalletFile)
{
...
CzHBETTracker::~CzHBETTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzHBETTracker::Init()
{
...
bool CzHBETTracker::Archive(CMintMeta& meta)
{
...
bool CzHBETTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzHBETTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzHBETTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzHBETTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzHBETTracker::GetSerialHashes()
{
...
CAmount CzHBETTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzHBETTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzHBETTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzHBETTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzHBETTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzHBETTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzHBETTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzHBETTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzHBETTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzHBETTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzHBETTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHBETTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHBETTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzHBETTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzHBETTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzHBETTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzHBETTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzhbet from DB\n", __func__, listDeterministicDB.size());
    }
