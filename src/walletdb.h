...
public:
    ...
    bool WriteZHBETSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHBETSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHBETSeed_deprecated(uint256& seed);
    bool EraseZHBETSeed();
    bool EraseZHBETSeed_deprecated();

    bool WriteZHBETCount(const uint32_t& nCount);
    bool ReadZHBETCount(uint32_t& nCount);
