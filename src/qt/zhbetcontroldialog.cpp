...
#include "ui_zhbetcontroldialog.h"
...
std::set<std::string> ZHbetControlDialog::setSelectedMints;
std::set<CMintMeta> ZHbetControlDialog::setMints;

ZHbetControlDialog::ZHbetControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZHbetControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZHbetControlDialog::~ZHbetControlDialog()
{
    delete ui;
}

void ZHbetControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZHbetControlDialog::updateList()
{
...
void ZHbetControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZHbetControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZHbet_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZHbetControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZHbetControlDialog::GetSelectedMints()
{
...
void ZHbetControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
