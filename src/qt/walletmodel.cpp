...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered hoolibet address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 HBET.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal hoolibet:URI (hoolibet:XyZ...?message=example)
            {
...
static void NotifyzHBETReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzHBETReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzHBETReset.connect(boost::bind(NotifyzHBETReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzHBETReset.disconnect(boost::bind(NotifyzHBETReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zhbetTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
