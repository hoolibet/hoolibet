...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::HBET);
    ...
    if (!settings.contains("nAnonymizeHooliBetAmount"))
        settings.setValue("nAnonymizeHooliBetAmount", 1000);

    nAnonymizeHooliBetAmount = settings.value("nAnonymizeHooliBetAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeHooliBetAmount"))
        SoftSetArg("-anonymizehoolibetamount", settings.value("nAnonymizeHooliBetAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in hoolibet.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHooliBetAmount:
            return QVariant(nAnonymizeHooliBetAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeHooliBetAmount:
            nAnonymizeHooliBetAmount = value.toInt();
            settings.setValue("nAnonymizeHooliBetAmount", nAnonymizeHooliBetAmount);
            emit anonymizeHooliBetAmountChanged(nAnonymizeHooliBetAmount);
            break;
        case CoinControlFeatures:
