...
//
// Bootup the Masternode, look for a 5000 HooliBet input and register on the network
//
...
vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].nValue == GetMNCollateral(chainActive.Height()) * COIN || out.tx->vout[out.i].nValue == GetMNCollateralOld(chainActive.Height()) * COIN)
